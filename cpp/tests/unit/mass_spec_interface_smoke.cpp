...
int main() {
    ...
    for (const auto &definition : operations.list("mass_spec")) {
        ...
        const auto expected_module = definition.id.find("chromatograms") != std::string::npos
                                         ? "mass_spec.chromatograms"
                                         : (definition.id.find("features") != std::string::npos ||
                                            definition.id.find("suspects") != std::string::npos ||
                                            definition.id.find("internal_standards") != std::string::npos ||
                                            definition.id.find("transformation_products") != std::string::npos
                                                ? "mass_spec.nta"
                                                : "mass_spec.base");
        if (found->value("module_id", "") != expected_module) {
            std::cerr << "mass-spec interface: module mismatch for " << definition.id << '\n';
            return 1;
        }
