...
void install_nta_schema(streamfind::ProjectTableStore &tables) {
    const auto manifest = catalogue::table_manifest_json("mass_spec", "mass_spec.nta");
    if (!manifest) throw Error(ErrorCode::DatabaseError, "NTA table manifest unavailable");
    const auto sql_type = [](const std::string &type) {
        if (type == "string") return std::string("VARCHAR");
        if (type == "integer") return std::string("INTEGER");
        if (type == "real") return std::string("DOUBLE");
        if (type == "boolean") return std::string("BOOLEAN");
        if (type == "timestamp") return std::string("TIMESTAMP");
        throw Error(ErrorCode::SchemaMismatch, "unsupported NTA semantic column type: " + type);
    };
    for (const auto &table : *manifest) {
        const auto name = table.value("table_name", "");
        std::string columns;
        for (const auto &column : table.value("columns", Json::array())) {
            if (!columns.empty()) columns += ", ";
            const auto column_name = column.value("name", "");
            columns += "\"" + column_name + "\" " + sql_type(column.value("type", ""));
            if (column_name == "created_at") columns += " DEFAULT CURRENT_TIMESTAMP";
        }
        tables.ensure_table(name, "CREATE TABLE IF NOT EXISTS \"" + name + "\" (" + columns + ")");
    }
}
namespace detail {
