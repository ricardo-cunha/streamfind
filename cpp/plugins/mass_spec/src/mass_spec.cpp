...
namespace streamfind::mass_spec
{
    ...
    Project::Project(streamfind::Project &project) : project_(project) {}

    void install_base_schema(streamfind::ProjectTableStore &tables)
    {
    ...
    {
        streamfind::ProjectTableStore::transaction(project_,
            {"MASS_SPEC_ANALYSES", "MASS_SPEC_CHROMATOGRAMS_HEADERS", "MASS_SPEC_SPECTRA_HEADERS"},
            [](streamfind::ProjectTableStore &tables) { install_base_schema(tables); });
    }

    Json Project::add_analyses(const Json &parameters)
    {
        ProjectTableStore(project_).require_installed_manifest("mass_spec", "mass_spec.base");
        Json added = Json::array();
        std::vector<std::vector<std::optional<std::string>>> analysis_rows;
        ...
        if (!analysis_rows.empty())
            project_.append_rows("MASS_SPEC_ANALYSES",
                {"analysis", "analysis_index", "source_analysis_number", "analysis_count", "replicate", "blank",
                 ...
                 "start_rt", "end_rt", "has_ion_mobility", "concentration"}, analysis_rows);
        return added;
    }
    ...
    Json Project::remove_analyses(const Json &parameters)
    {
        ...
        for (const auto &value : parameters.at("analysis_names")) {
            ...
            names += detail::sql(name);
            removed.push_back(name);
        }
        if (!names.empty()) project_.execute_sql("DELETE FROM MASS_SPEC_ANALYSES WHERE analysis IN (" + names + ")");
        return removed;
    }
    ...
    Json update_analysis_values(streamfind::Project &project, const Json &parameters, const char *key, const char *column)
    {
        ...
        if (names.empty()) return Json{{"updated", 0}};
        std::string expression = "CASE analysis";
        for (std::size_t i = 0; i < names.size(); ++i) {
            const auto analysis = names[i].at("analysis").get<std::string>();
            const auto value = values[i];
            const auto replacement = std::string(column) == "concentration"
                ? std::to_string(value.get<double>()) : detail::sql(value.get<std::string>());
            expression += " WHEN " + detail::sql(analysis) + " THEN " + replacement;
        }
        expression += " END";
        project.execute_sql(std::string("UPDATE MASS_SPEC_ANALYSES SET ") + column + " = " + expression);
    ...
    {
        ProjectTableStore(project_).require_installed_manifest("mass_spec", "mass_spec.chromatograms");
    ...
    {
        ProjectTableStore(project).require_installed_manifest("mass_spec", "mass_spec.nta");
        const double ppm = p.value("ppm", 20.0);
        const double rt_tolerance = p.value("rt_tolerance", 60.0);
        if (ppm < 0.0 || rt_tolerance < 0.0)
            throw streamfind::Error(streamfind::ErrorCode::InvalidArgument, "ppm and rt_tolerance must be non-negative");
